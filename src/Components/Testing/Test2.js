...
import { SampleBase } from "./sample_base";
import { UploaderComponent } from "@syncfusion/ej2-react-inputs";
import { detach } from "@syncfusion/ej2-base";

class Customdroparea extends SampleBase {
  constructor(props) {
    super(props);
    ...
    return (
      <span>
        <span className="fileListwrapper">
          <span className={`icon template-icons sf-icon-${data.type}`}></span>
          <span className="upload-name file-name">
            {data.name} ({data.size} bytes)
          </span>
          <span className="upload-status">{data.status}</span>
        </span>
          ...
          <div className="col-lg-12 control-section upload-custom">
            <div className="customdrop_wrapper">
              <div className="dropArea_wrap" id="customTarget">
                ...
                <span className="dropText" id="dropText">
                  Drop files here to upload
                </span>
              </div>
              <div id="customdropArea">
                <span id="drop" className="customdropArea">
                  <a href="" id="browse">
                    <u>Browse</u>
                  </a>{" "}
                </span>
